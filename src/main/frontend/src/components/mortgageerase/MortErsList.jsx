...
const MortErsList = () => {
    ...
    const fetchMortErsList = async () => {
        try {
            ...
            const response = await axios.post('/api/mortgageerase/list', params);
            if (response.data.success) {
                setRowData(response.data.list);
                setTotalCount(response.data.list.length);
            }
        } catch (error) {
            console.error('저당말소현황 조회 실패:', error);
    ...
    const fetchCodes = async () => {
        try {
            const groupIds = ['SGB', 'PR_ST', 'PAYST', 'GOVT'];
    ...
    const handleSearchClick = () => fetchMortErsList();
    ...
    const handleExportExcel = () => {
        if (gridRef.current && gridRef.current.api) {
            gridRef.current.api.exportDataAsCsv({ fileName: `저당말소현황_${new Date().toISOString().split('T')[0]}.csv` });
        }
    };
        ...
        <div className="status-container">
            ...
            <div className="status-toolbar">
                <div className="toolbar-left">
                    <span className="toolbar-title">저당말소현황</span>
                ...
                <div className="erp-row">
                    <ErpField label="신청구분" span={5}>
                        <select className="erp-input" value={searchFilters.workCode} onChange={e => setSearchFilters({...searchFilters, workCode: e.target.value})}>
                            <option value="ERS">저당말소 (전체)</option>
                            <option value="ERS_1">일반말소</option>
                        </select>
            ...
            <div className="grid-container ag-theme-alpine" style={{ height: 'calc(100% - 180px)', width: '100%' }}>
                <AgGridReact
                    ...
                    onRowDoubleClicked={(e) => navigate('/mortgageerase/mort-ers-request', { state: { receiptNo: e.data.SERVICE_ID } })}
                />
            </div>
        </div>
    );
