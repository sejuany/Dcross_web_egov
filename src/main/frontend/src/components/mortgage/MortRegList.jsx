import React, { useState, useRef, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
import './MortRegList.css';
import { AgGridReact } from 'ag-grid-react';
import { ModuleRegistry, AllCommunityModule } from 'ag-grid-community';
import 'ag-grid-community/styles/ag-grid.css';
import 'ag-grid-community/styles/ag-theme-alpine.css';
import { useAuth } from '../../context/AuthContext';
import ErpSection from '../common/ErpSection';
import ErpField from '../common/ErpField';

// Register AG Grid modules
ModuleRegistry.registerModules([AllCommunityModule]);

const getFormattedDateOffset = (offsetDays) => {
    const d = new Date();
    d.setDate(d.getDate() + offsetDays);
    const year = d.getFullYear();
    const month = String(d.getMonth() + 1).padStart(2, '0');
    const day = String(d.getDate()).padStart(2, '0');
    return `${year}-${month}-${day}`;
};

const MortRegList = () => {
    const navigate = useNavigate();
    const gridRef = useRef(null);
    const { user } = useAuth();

    const [codeMap, setCodeMap] = useState({});
    const [codeListMap, setCodeListMap] = useState({});
    const [companyList, setCompanyList] = useState([]);
    const [toastMessage, setToastMessage] = useState('');
    const [rowData, setRowData] = useState([]);
    const [totalCount, setTotalCount] = useState(0);

    const [searchFilters, setSearchFilters] = useState({
        workCode: '000',
        companyId: '',
        govtId: '',
        carNo: '',
        mortgagorNm: '',
        startDate: getFormattedDateOffset(0),
        endDate: getFormattedDateOffset(0),
        processStatus: '전체',
        payStatus: '전체'
    });

    const fetchMortgageList = async () => {
        try {
            const cleanParam = (val) => (val === '전체' || val === '전체 (회사)' || val === '전체 (관청)') ? '' : val;

            const params = {
                WORK_CD: searchFilters.workCode,
                COMPANY_ID: cleanParam(searchFilters.companyId),
                GOVT_ID: cleanParam(searchFilters.govtId),
                CAR_NO: searchFilters.carNo,
                MORTGAGOR_NM: searchFilters.mortgagorNm,
                START_DT: searchFilters.startDate.replace(/-/g, ''),
                END_DT: searchFilters.endDate.replace(/-/g, ''),
                PROC_ST: cleanParam(searchFilters.processStatus)
                // PAY_ST: cleanParam(searchFilters.payStatus) // Add if backend supports it
            };

            const response = await axios.post('/api/mortgage/list', params);
            if (response.data.success) {
                setRowData(response.data.list);
                setTotalCount(response.data.list.length);
            }
        } catch (error) {
            console.error('설정신청현황 조회 실패:', error);
            setToastMessage('데이터 조회에 실패했습니다.');
            setTimeout(() => setToastMessage(''), 2500);
        }
    };

    const fetchCodes = async () => {
        try {
            const groupIds = ['PR_ST', 'PAYST', 'GOVT'];
            const responses = await Promise.all(
                groupIds.map(id => axios.get(`/api/codes/${id}`))
            );

            const newCodeMap = {};
            const newCodeListMap = {};
            responses.forEach((res, index) => {
                const groupId = groupIds[index];
                if (res.data.success) {
                    newCodeListMap[groupId] = res.data.codes;
                    const tempMap = {};
                    res.data.codes.forEach(code => {
                        tempMap[code.CODE_ID] = code.CODE_NM;
                    });
                    newCodeMap[groupId] = tempMap;
                }
            });
            setCodeMap(newCodeMap);
            setCodeListMap(newCodeListMap);
        } catch (error) {
            console.error('공통 코드 조회 실패:', error);
        }
    };

    const fetchCompanies = async () => {
        try {
            const response = await axios.get('/api/companies', {
                params: { workCd: '000' }
            });
            if (response.data.success) {
                setCompanyList(response.data.list);
            }
        } catch (error) {
            console.error('회사 목록 조회 실패:', error);
        }
    };

    useEffect(() => {
        fetchCodes();
        fetchCompanies();
        fetchMortgageList();
    }, []);

    const formatCode = (groupId, value) => {
        return codeMap[groupId] && codeMap[groupId][value] ? codeMap[groupId][value] : value;
    };

    const columnDefs = [
        { headerCheckboxSelection: true, checkboxSelection: true, width: 40, pinned: 'left' },
        { headerName: '접수번호', field: 'SERVICE_ID', width: 150 },
        { headerName: '공동', field: 'JOINT_YN', width: 60, cellClass: 'text-center' },
        { headerName: '차량번호', field: 'CAR_NO', width: 110 },
        { headerName: '설정자명', field: 'MORTGAGOR_NM', width: 120 },
        { headerName: '채권가액', field: 'BOND_AMT', width: 110, type: 'numericColumn', valueFormatter: params => params.value?.toLocaleString() },
        { headerName: '납부금액', field: 'PAY_AMT', width: 100, type: 'numericColumn', valueFormatter: params => params.value?.toLocaleString() },
        { headerName: '처리상태', field: 'PROC_ST', width: 90, valueFormatter: params => formatCode('PR_ST', params.value) },
        { headerName: '자동', field: 'AUTO_YN', width: 60, cellClass: 'text-center' },
        {
            headerName: '관청',
            field: 'GOVT_ID',
            width: 80,
            valueFormatter: params => formatCode('GOVT', params.value),
            cellClassRules: {
                'bg-pink': params => params.value === 'DG', // 대구
                'bg-mint': params => ['HY', 'HA', 'CW'].includes(params.value), // 함양, 함안, 창원
            }
        },
        { headerName: '심사요청시간', field: 'JUDGE_REQ_TIME', width: 130 },
        { headerName: '서명방법', field: 'SIGN_METHOD', width: 130 }
    ];

    const handleSearchClick = () => fetchMortgageList();
    const handleResetClick = () => {
        setSearchFilters({
            workCode: '000',
            companyId: '',
            govtId: '',
            carNo: '',
            mortgagorNm: '',
            startDate: getFormattedDateOffset(0),
            endDate: getFormattedDateOffset(0),
            processStatus: '전체',
            payStatus: '전체'
        });
    };
    const handleExportExcel = () => {
        if (gridRef.current?.api) {
            gridRef.current.api.exportDataAsCsv({ fileName: `설정신청현황_${new Date().toISOString().split('T')[0]}.csv` });
        }
    };
    const handleCloseClick = () => navigate('/home');

    useEffect(() => {
        const handleKeyDown = (e) => {
            if (e.key === 'F2') { e.preventDefault(); handleSearchClick(); }
            if (e.key === 'F7') { e.preventDefault(); handleExportExcel(); }
            if (e.key === 'F8') { e.preventDefault(); handleResetClick(); }
            if (e.key === 'F9') { e.preventDefault(); handleCloseClick(); }
        };
        window.addEventListener('keydown', handleKeyDown);
        return () => window.removeEventListener('keydown', handleKeyDown);
    }, [searchFilters]);

    return (
        <div className="status-container">
            {toastMessage && <div className="toast-notification">{toastMessage}</div>}

            <div className="status-toolbar">
                <div className="toolbar-left">
                    <button className="btn-status grey">등록[F10]</button>
                    <span className="title-count">{totalCount}</span> 건이 조회 됩니다.
                </div>
                <div className="toolbar-right">
                    <button className="btn-status" onClick={handleSearchClick}>조회[F2]</button>
                    <button className="btn-status" onClick={handleExportExcel}>엑셀[F7]</button>
                    <button className="btn-status" onClick={handleResetClick}>초기화[F8]</button>
                    <button className="btn-status" onClick={handleCloseClick}>닫기[F9]</button>
                </div>
            </div>

            <ErpSection isHeader={true}>
                <div className="erp-row">
                    <ErpField label="신청구분" span={5}>
                        <select className="erp-input" value={searchFilters.workCode} onChange={e => setSearchFilters({ ...searchFilters, workCode: e.target.value })}>
                            <option value="MORTREG">전체</option>
                        </select>
                        <select className="erp-input" value={searchFilters.companyId} onChange={e => setSearchFilters({ ...searchFilters, companyId: e.target.value })}>
                            <option value="">전체</option>
                            {companyList.map(comp => <option key={comp.COMPANY_ID} value={comp.COMPANY_ID}>{comp.COMPANY_NM}</option>)}
                        </select>
                        <select className="erp-input" value={searchFilters.govtId} onChange={e => setSearchFilters({ ...searchFilters, govtId: e.target.value })}>
                            <option value="">전체</option>
                            {codeListMap['GOVT']?.map(code => <option key={code.CODE_ID} value={code.CODE_ID}>{code.CODE_NM}</option>)}
                        </select>
                    </ErpField>
                    <ErpField label="차량번호" span={3}>
                        <input type="text" className="erp-input" value={searchFilters.carNo} onChange={e => setSearchFilters({ ...searchFilters, carNo: e.target.value })} />
                    </ErpField>
                    <ErpField label="설정자명" span={4}>
                        <input type="text" className="erp-input" value={searchFilters.mortgagorNm} onChange={e => setSearchFilters({ ...searchFilters, mortgagorNm: e.target.value })} />
                    </ErpField>
                </div>
                <div className="erp-row">
                    <ErpField label="신청일자" span={5}>
                        <input type="date" className="erp-input" value={searchFilters.startDate} onChange={e => setSearchFilters({ ...searchFilters, startDate: e.target.value })} />
                        <span>~</span>
                        <input type="date" className="erp-input" value={searchFilters.endDate} onChange={e => setSearchFilters({ ...searchFilters, endDate: e.target.value })} />
                    </ErpField>
                    <ErpField label="처리상태" span={3}>
                        <select className="erp-input" value={searchFilters.processStatus} onChange={e => setSearchFilters({ ...searchFilters, processStatus: e.target.value })}>
                            <option value="전체">전체</option>
                            {codeListMap['PR_ST']?.map(code => <option key={code.CODE_ID} value={code.CODE_ID}>{code.CODE_NM}</option>)}
                        </select>
                    </ErpField>
                    <ErpField label="납부상태" span={4}>
                        <select className="erp-input" value={searchFilters.payStatus} onChange={e => setSearchFilters({ ...searchFilters, payStatus: e.target.value })}>
                            <option value="전체">전체</option>
                            {codeListMap['PAYST']?.map(code => <option key={code.CODE_ID} value={code.CODE_ID}>{code.CODE_NM}</option>)}
                        </select>
                    </ErpField>
                </div>
            </ErpSection>

            <div className="grid-container ag-theme-alpine">
                <AgGridReact
                    ref={gridRef}
                    rowData={rowData}
                    columnDefs={columnDefs}
                    defaultColDef={{ sortable: true, resizable: true }}
                    rowSelection="multiple"
                    headerHeight={28}
                    rowHeight={24}
                />
            </div>
        </div>
    );
};

export default MortRegList;
